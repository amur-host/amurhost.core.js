describe('LocalStorage.chrome.Service', function () {
    ...
    beforeEach(function () {
        window.chrome = {
            storage: {
                sync: {
                    set: function() {},
                    get: function(key, cb) { cb({'AmurAccounts': 'value'}); }
                },
                local: {
                    set: function() {},
                    get: function(key, cb) { cb({'AmurAccounts': 'value'}); }
    ...
    it('should save state', function () {
        ...
        //then
        expect(window.chrome.storage.local.set).toHaveBeenCalledWith({'AmurAccounts': 'value'}, jasmine.any(Function));
    });
    ...
    it('should get state when sync state is empty', inject(function ($rootScope) {
        ...
        $rootScope.$digest();
        //then
        expect(window.chrome.storage.sync.get).toHaveBeenCalledWith('AmurAccounts', jasmine.any(Function));
        expect(window.chrome.storage.local.get).toHaveBeenCalledWith('AmurAccounts', jasmine.any(Function));
        expect(response).toEqual('value');
    }));

    it('should get state when sync state is not empty', inject(function ($rootScope) {
        var dataObject1 = {'AmurAccounts': 'value1'};
        var response;
